...
def load_and_save(symbol):
    try:
        ...
        if df.empty:
            ...
            return
        df.to_sql('finnhub_us_day_candle', lib.db_wrap.get_connection(), if_exists='append', index=False)
...
us_symbols_saved = pd.read_sql('select distinct symbol from finnhub_us_day_candle', lib.db_wrap.get_connection())
