...
def load_and_save(symbol):
    try:
        df = lib.finnhub_wrap.get_split(symbol)
        df['symbol'] = symbol
        if df.empty:
            print('>>>', symbol, 'no_data')
            return
        df.to_sql('finnhub_us_split', lib.db_wrap.get_connection(), if_exists='append', index=False)
...
us_symbols_saved = pd.read_sql('select distinct symbol from finnhub_us_split', lib.db_wrap.get_connection())
