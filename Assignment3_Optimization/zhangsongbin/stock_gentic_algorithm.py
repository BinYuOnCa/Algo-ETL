...
import stock_pgfunctions as pg
...
class Gentic():
    ...
    def main(self) -> dict:
        for you_want_list_length in range(self.long[0], self.long[1]):
            for change_days in range(self.d[0], self.d[1]):
                ...
                # 下面开始一个一个列表开始计算回报率
                df = pd.DataFrame(columns=["color_period", "period", "return_rate"])
                for i_list in range(len(you_want_list)):
                    ...
                    # print(f"原始颜色列表是{a[i_list]}，周期列表是：{period}，收益率是{return_avg}")
                    if return_avg > 0:
    ...
    def number_transfer(list_a, num, direct):
        ...
        :param direct: int   只能1或-1， 如果是1，表示从[0,1,2,3]到[8,9,10,11]。如果-1，表示[8,9,10,11]到[0,1,2,3]
    ...
    def one_list_return(self, one_list: list):
        ...
        :return period: list  这是类似[6, 8, 6, 8]格式的列表，里面是不同阶段的天数，实现d3-d2-d1-d0-d4周期
        ...
        # try:
        rr_class = ReturnRate(symbol, self.start_date, self.end_date, period_real)
    ...
    def one_list_return2(self, one_list: list):
        ...
        """
        symbol = pg.get_symbol_df(self.start_date, self.end_date)
        rr_class = ReturnRate(symbol, self.start_date, self.end_date, one_list)
        return_rr = rr_class.validate_three_plus_one_stocks()
        return return_rr
